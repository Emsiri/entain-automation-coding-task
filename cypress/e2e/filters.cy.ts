...
describe('Category Filters', () => {
  ...
  let raceNames = [];
  cy.get("[data-testid=cypress-race-name]").each(($el) => {
    ...
    cy.fixture('setRaces.json').then((races) => {
      ...
      const filteredEntries = Object.entries(races.race_summaries).filter(([_, value]) => raceNames.includes(value.meeting_name));
      // Get the category ids for assertion
      const categoryIds = filteredEntries.map(([_, value]) => value.category_id);
